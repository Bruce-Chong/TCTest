...
def set_linesource(zmatnr, zpo, zitm, zfci,zuom, zsubline,zrid, zqty, ztcci):
    line_source = """                           <LineItem Key=\"{7}_{1}\">
                        \r\n                                        <Attribute AttributeTypeCd=\"CC\">OR</Attribute>
                        \r\n                                        <Attribute AttributeTypeCd=\"PL\">00100</Attribute>
                        \r\n                                        <Attribute AttributeTypeCd=\"BP\">{2}</Attribute>
                        \r\n                                        <Attribute AttributeTypeCd=\"SW\">01000</Attribute>
                        \r\n                                        <Reference SourceRefTypeCd=\"128\" RefTypeCd=\"BAF\">{3}</Reference>
                        \r\n                                        <Reference SourceRefTypeCd=\"128\" RefTypeCd=\"4H\">{9}</Reference>
                        \r\n                                        <Reference SourceRefTypeCd=\"128\" RefTypeCd=\"4B\">JO</Reference>
                        \r\n                                        <Date TimeZone=\"UTC\" DateTypeCd=\"003\">{4}</Date>
                        \r\n                                        <Measure Qualifier=\"SQ\" SourceQualifier=\"738\" SourceUOMCd=\"355\" UOMCd=\"{5}\">{8}</Measure>
                        ...
                        \r\n{6}
                        \r\n                                    </LineItem>""".format(zpo, zitm, zmatnr,zfci, today_dt, zuom, zsubline, zrid, zqty, ztcci)

    return line_source
def post_api(po, ctrl_no, uuid, fci, zfty, zrid, ztcci):
    ...
    for row in ekpo_sheet.iter_rows():
        ...
        if str(EBELN) == po:
            if EBELP[-2:] == "00":
                ...
                if MASTER_ITM != "":
                    ttl_line = ttl_line + set_linesource(MATNR, po, MASTER_ITM, fci, UOM, ttl_subline, zrid, MASTER_QTY, ztcci)
                    ttl_subline = ""
                MASTER_ITM = EBELP.zfill(5)
                MATNR = is_number(row[7].value)
                UOM = is_number(row[18].value)
                MASTER_QTY = is_number(row[17].value)
                ...
                subline_source = """\r\n                                <Subline Key=\"{0}_{1}{2}\">
                        ...
                        \r\n                                        </Subline>""".format(zrid, MASTER_ITM, size[1].replace(" ", ""), UOM, QTY)
                if ttl_subline == "":
    ...
    if ttl_line != "":
        ...
        ttl_line = set_linesource(MATNR, po, MASTER_ITM, fci, UOM, ttl_subline, zrid, MASTER_QTY, ztcci)
...
with st.form("Create OR"):
    ...
    in_TCCI = st.text_input("TCCI")
    in_RID = st.text_input("RID")
    ...
    if submitted:
        try:
            post_api(str(in_PO), str(in_ctrl), str(in_UUID), str(in_FCI), in_FTY, str(in_RID), str(in_TCCI))
